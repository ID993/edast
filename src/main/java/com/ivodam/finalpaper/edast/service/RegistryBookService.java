...
public class RegistryBookService {
  ...
  public String classNumberGenerator(String requestName) {
    return "611-03/" + LocalDate.now().getYear() + "-" +
        requestName.substring(0, 3).toUpperCase() + "/" +
  ...
  public void divideRequests(UUID userId) {
    var registryBooks =
        registryBookRepository.findByEmployeeIdAndStatus(userId, "In progress");
  ...
  public String getDateDay(long days) {
    if (LocalDate.now().getDayOfWeek() == DayOfWeek.SATURDAY)
      return LocalDate.now().plusDays(days + 2).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
    else if (LocalDate.now().getDayOfWeek() == DayOfWeek.SUNDAY)
      return LocalDate.now().plusDays(days + 1).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
    else
      return LocalDate.now().plusDays(days).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
  }

  public String getDeadlineDate() {
    if (LocalDate.now().getDayOfWeek() == DayOfWeek.SATURDAY)
      return LocalDate.now().plusDays(17).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
    else if (LocalDate.now().getDayOfWeek() == DayOfWeek.SUNDAY)
      return LocalDate.now().plusDays(16).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
    else
      return LocalDate.now().plusDays(15).format(
          DateTimeFormatter.ofPattern(DATE_FORMAT));
  }
  ...
  public void deleteById(UUID id) { registryBookRepository.deleteById(id); }

  public RegistryBook findById(UUID id) throws AppException {
    return registryBookRepository.findById(id).orElseThrow(
        ()
            -> new AppException("Record with id: " + id + " is not found",
                                HttpStatus.NOT_FOUND));
  }

  public void reassignToArchivist(UUID registryBookId, UUID employeeId)
      throws AppException {

    var registryBook = findById(registryBookId);
    var employee = userService.findById(employeeId);

    if (employee.getRole() != Enums.Roles.ROLE_EMPLOYEE ||
        !"Archivist".equals(employee.getJobTitle())) {
      throw new AppException("Selected user is not an archivist",
  ...
  public RegistryBook findByRequestId(UUID id) throws AppException {
    return registryBookRepository.findByRequestId(id).orElseThrow(
        ()
            -> new AppException("Record with request id: " + id +
                                    " is not found",
                                HttpStatus.NOT_FOUND));
  }

  public Page<RegistryBook> findByEmployeeIdAndKeyword(UUID id, String keyword,
                                                       Pageable pageable) {
    return registryBookRepository.findByEmployeeIdAndKeyword(id, keyword,
                                                             pageable);
  }

  public void updateRequestByName(UUID requestId, String requestName)
      throws AppException {
    switch (requestName) {
    case "Registry" -> {
      var bdmRequest = bdmRequestRepository.findById(requestId).orElseThrow(
          ()
              -> new AppException("Request with id: " + requestId +
                                      " not found",
                                  HttpStatus.NOT_FOUND));
    ...
    case "Education" -> {
      var educationRequest =
          educationRequestRepository.findById(requestId).orElseThrow(
              ()
                  -> new AppException("Request with id: " + requestId +
                                          " not found",
                                      HttpStatus.NOT_FOUND));
    ...
    case "Cadastral" -> {
      var cadastralRequest =
          cadastralRequestRepository.findById(requestId).orElseThrow(
              ()
                  -> new AppException("Request with id: " + requestId +
                                          " not found",
                                      HttpStatus.NOT_FOUND));
    ...
    default -> {
      var specialRequest =
          specialRequestRepository.findById(requestId).orElseThrow(
              ()
                  -> new AppException("Request with id: " + requestId +
                                          " not found",
                                      HttpStatus.NOT_FOUND));
  ...
  public void updateReadStatus(RegistryBook registryBook) {
    registryBook.setRead(true);
    registryBookRepository.save(registryBook);
  }

  public Page<RegistryBook>
  findByEmployeeIdAndRead(UUID employeeId, boolean read, Pageable pageable) {
    return registryBookRepository.findByEmployeeIdAndRead(employeeId, read,
                                                          pageable);
  }
  ...
  public long countByEmployeeIdAndStatus(UUID employeeId, String status) {
    return registryBookRepository.countByEmployeeIdAndStatus(employeeId,
                                                             status);
  }

  public Page<RegistryBook>
  searchByClassNumberOrUserOrEmployee(String keyword, Pageable pageable) {
    return registryBookRepository.searchAllByClassNumberOrUserOrEmployee(
        keyword, pageable);
  }

  public Page<RegistryBook> searchAllBdmByClassNumberOrUser(String keyword,
                                                            UUID employeeId,
                                                            Pageable pageable) {
    return registryBookRepository.searchAllBdmByClassNumberOrUser(
        keyword, employeeId, pageable);
  }

  public Page<RegistryBook>
  searchAllWorkByClassNumberOrUser(String keyword, UUID employeeId,
                                   Pageable pageable) {
    return registryBookRepository.searchAllWorkByClassNumberOrUser(
        keyword, employeeId, pageable);
  }

  public Page<RegistryBook>
  searchAllEducationByClassNumberOrUser(String keyword, UUID employeeId,
                                        Pageable pageable) {
    return registryBookRepository.searchAllEducationByClassNumberOrUser(
        keyword, employeeId, pageable);
  }

  public Page<RegistryBook>
  searchAllCadastralByClassNumberOrUser(String keyword, UUID employeeId,
                                        Pageable pageable) {
    return registryBookRepository.searchAllCadastralByClassNumberOrUser(
        keyword, employeeId, pageable);
  }

  public Page<RegistryBook>
  searchAllSpecialByClassNumberOrUser(String keyword, UUID employeeId,
                                      Pageable pageable) {
    return registryBookRepository.searchAllSpecialByClassNumberOrUser(
        keyword, employeeId, pageable);
  }

  public Page<RegistryBook>
  searchAllByClassNumberOrUserOrEmployeeAndRequestName(String keyword,
                                                       String requestName,
                                                       Pageable pageable) {
    return registryBookRepository
        .searchAllByClassNumberOrUserOrEmployeeAndRequestName(
            keyword, requestName, pageable);
  }
  ...
  public long count() { return registryBookRepository.count(); }
}
