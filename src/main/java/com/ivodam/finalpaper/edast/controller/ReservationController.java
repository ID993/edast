...
public class ReservationController {
  ...
  public String
  getReservations(Model model, @RequestParam(defaultValue = "") String keyword,
    ...
    if (user.getRole().equals(Enums.Roles.ROLE_USER))
      reservations =
          reservationService.findAllByUserId(user.getId(), keyword, pageable);
    else
  ...
  public String getReservationsOfToday(
      Model model, @RequestParam(defaultValue = "") String keyword,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "6") int size,
      @RequestParam(defaultValue = "dateCreated") String sortBy,
      @RequestParam(defaultValue = "asc") String sortOrder) {
    ...
    var pageable = PageRequest.of(page, size, sort);
    var reservations =
        reservationService.findAllByDateOfReservation(keyword, pageable);
    model.addAttribute("keyword", keyword);
    model.addAttribute("currentPage", page);
    model.addAttribute("sortBy", sortBy);
    model.addAttribute("sortOrder", sortOrder);
  ...
  public String makeReservation(@Valid @ModelAttribute Reservation reservation,
    ...
    if (result.hasErrors()) {
      return "reservation/make-reservation";
    } else if (reservationService.existsByUserAndDate(
                   user.getId(), reservation.getDateOfReservation())) {
  ...
  public String reservationDetails(@PathVariable UUID id, Model model)
      throws AppException {

    var reservation = reservationService.findAccessibleById(id, currentUser());

    var updatable = reservationService.canUpdateReservation(
  ...
  public String deleteReservation(@PathVariable UUID id) throws AppException {
    ...
    reservationService.deleteOwnedBy(id, currentUser().getId());
    return "redirect:/reservations";
  }

  @GetMapping("/reservation/{id}/edit")
  public String editReservation(@PathVariable UUID id, Model model)
      throws AppException {

    var reservation =
        reservationService.findUpdatableOwnedById(id, currentUser().getId());
  ...
  public String editReservation(@PathVariable UUID id,
    ...
    reservationService.updateOwnedBy(id, currentUser().getId(), reservation);
