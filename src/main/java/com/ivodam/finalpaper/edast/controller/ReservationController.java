...
public class ReservationController {
    ...
    public String makeReservation(@Valid @ModelAttribute Reservation reservation,
                                  BindingResult result, Model model) throws AppException, MessagingException {
        var user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        if (result.hasErrors()) {
            return "reservation/make-reservation";
        } else if (reservationService.existsByUserAndDate(user.getId(), reservation.getDateOfReservation())) {
            model.addAttribute("message", "You already have a reservation for this date!");
            return "reservation/make-reservation";
        }
