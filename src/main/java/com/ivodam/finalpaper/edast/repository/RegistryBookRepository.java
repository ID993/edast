...
public interface RegistryBookRepository extends JpaRepository<RegistryBook, UUID> {
    ...
    Page<RegistryBook> findByEmployeeId(UUID employeeId, Pageable pageable);
    ...
    @Query("SELECT e FROM registry_book e WHERE LOWER(e.classNumber) LIKE %:keyword% OR LOWER(e.user.name) LIKE %:keyword% AND e.employee.id IN (SELECT u.id FROM users u WHERE u.id = :employeeId)")
    Page<RegistryBook> searchAllByClassNumberOrUser(String keyword, UUID employeeId, Pageable pageable);
