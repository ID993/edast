...
public interface BDMRequestRepository extends JpaRepository<BDMRequest, UUID> {
        ...
        @Query("SELECT e FROM bdm_request e WHERE (LOWER(e.bdmSelection) LIKE %:keyword% OR LOWER(e.fullName) LIKE %:keyword%) AND e.user.id IN (SELECT u.id FROM users u WHERE u.id = :userId)")
        List<BDMRequest> searchAllByKeyword(String keyword, UUID userId);
